...
const {checkinAnalysis} = require("../Service/checkinAnalysis");
const {
  calculateMoodScore,
...
const createCheckIn = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(400)
        .json({ success: false, message: "User Not detected" });
    }
    ...
    if (!checkInCreate) {
      return res
        .status(400)
        .json({ success: false, message: "Failed To Create Check In" });
    }
console.log("AI CHECK-IN: about to call AI");
    try {
        console.log("AI CHECK-IN: about to call AI");
      const analysis = await checkinAnalysis({
        moodScore,
        energyScore,
      ...
      checkInCreate.analysis = analysis;
      await checkInCreate.save();
    } catch (error) {
...
const getTodayCheckIns = async (req, res) => {
  try {
    ...
    const latestCheckIns = await checkinModel
      .findOne({ userId })
      .sort({ timestamp: -1 });
...
const getCheckInById = async (req,res) =>{
  try {
    ...
    const getCheckIn = await checkinModel.findById(id)
    return res.status(200).json({
      success:true,
      message:'Succeful Fetched',
      checkInData:getCheckIn
    })
  } catch (error) {
     console.error(error);
    return res.status(500).json({
...
module.exports = { createCheckIn, getCheckInHistory, getTodayCheckIns,getCheckInById };
